...
func (m *Metadata) DecryptKey(ek, tpmKey []byte) ([]byte, error) {
	if len(ek) != len(m.kek) {
...
func (m *Metadata) EncryptKey(key, tpmKey []byte) ([]byte, error) {
	return m.DecryptKey(key, tpmKey)
}
