...
| Name               | Description                                                            | Type    | Labels                                                | Notes          |
| ------------------ | ---------------------------------------------------------------------- | ------- | ----------------------------------------------------- | -------------- |
| machine_status     | The machine status (see [Machine States](lifecycle.md#Machine-States)) | Gauge   | status, address, serial, rack, role, machine_type (1) | leaderOnly (2) |
...
(1) "machine_type" is derived from [the user-defined `labels`](machine.md#machinespec-struct) with the key of `machine-type`.
(2) The leaderOnly metrics are exposed only by the instance that has been elected as the leader.
