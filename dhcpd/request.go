...
func (h DHCPHandler) handleRequest(ctx context.Context, pkt *dhcp4.Packet, intf Interface) (*dhcp4.Packet, error) {
	...
	if hasServerIdentifier {
		...
		if !serverAddr.Equal(serverIdentifier) {
			// not chosen
			log.Info("dhcp: ignored request to another server", addPacketLog(pkt, map[string]any{
	...
	if hasRequestedIP {
		// case 2.
		log.Info("dhcp: requested confirmation on reboot", addPacketLog(pkt, map[string]any{
			optionLogKey(dhcp4.OptRequestedIP): requestedIP,
		}))

		err = h.DHCP.Renew(ctx, requestedIP, pkt.HardwareAddr)
		if err != nil {
			log.Warn("dhcp: requested confirmation but found no record", addPacketLog(pkt, map[string]any{
				optionLogKey(dhcp4.OptRequestedIP): requestedIP,
			}))
	...
	// case 3.
	log.Info("dhcp: requested renewal", addPacketLog(pkt, map[string]any{
		pktCiaddr: pkt.ClientAddr,
	}))

	err = h.DHCP.Renew(ctx, pkt.ClientAddr, pkt.HardwareAddr)
	if err != nil {
		log.Warn("dhcp: requested renewal but found no record", addPacketLog(pkt, map[string]any{
			pktCiaddr: pkt.ClientAddr,
		}))
		return nil, errNoRecord
	}
